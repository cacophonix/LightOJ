...
const int MAX = (1<<18);
const int BUFF = 0x800000;

struct NODE { int d[3], flip; };

NODE Tree[MAX];
char buff[BUFF], *ptr = buff;

inline int nextInt() {
	int ret = 0;
	while(*ptr < 48) ptr++;
	do { ret = ret * 10 + *ptr++ - 48; } while(*ptr >= 48);
	return ret;
}
...
int main() {
	int n, q, t, i, j, test, cs;
	fread_unlocked(buff, 1, BUFF, stdin);
	test = nextInt();
	for(cs = 1; cs <= test; cs++) {
		n = nextInt(); q = nextInt();
		CLR(Tree); init(1, 0, n-1);
		printf("Case %d:\n", cs);
		while(q--) {
			t = nextInt(); i = nextInt(); j = nextInt();
			switch(t) {
